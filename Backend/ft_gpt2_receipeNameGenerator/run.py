...
from model import load_model_and_tokenizer, generate_recipes, clean_and_split
...
def _parse_args():
    ...
    p.add_argument("--live_eval", action="store_true", help="Run in live evaluation mode for chatbot")
    ...
    p.add_argument("--ingredients", type=str, help="Comma-separated ingredients (for live eval)")
    p.add_argument("--tags", type=str, help="Comma-separated tags (for live eval)")
...
def _build_trainer(args):
    ...
    # DEBUG: print token id info for sample 0
    if len(ds_train.records) > 0:
        rec = ds_train.records[0]
        prompt = ds_train._build_prompt(rec)
        full = prompt + ds_train._build_label_seq(rec) + tok.eos_token
    ...
    targs = TrainingArguments(
        ...
        fp16=args.fp16 and not args.cpu,  # Disable fp16 if CPU is forced
        report_to="none",
        save_total_limit=5,
...
def main():
    ...
    if args.cpu:
        print("🖥️  Forcing CPU usage as requested (--cpu flag)")
    else:
        device = "CUDA" if torch.cuda.is_available() else "CPU"
        gpu_count = torch.cuda.device_count() if torch.cuda.is_available() else 0
        gpu_names = [torch.cuda.get_device_name(i) for i in range(gpu_count)] if gpu_count > 0 else []
        print(f"🖥️  Using {device} device{'s' if gpu_count > 1 else ''}: {', '.join(gpu_names) if gpu_names else 'CPU only'}")
    
    # Live evaluation mode for chatbot integration
    if args.live_eval:
        if not args.ingredients or not args.tags:
            print("Error: For live evaluation, you must provide --ingredients and --tags")
            sys.exit(1)
            
        print(f"🔍 Running live evaluation using model: {args.model}")
        ...
        # Parse ingredients and tags from command line
        ...
        recipe_names = generate_recipes(model, tokenizer, ingredients, tags, max_length=100, live_eval=True)
    ...
    if trainer.is_world_process_zero():
        ...
        df.to_csv(Path(args.output) / "eval_manual.csv", index=False)
        print(f"▶ Manual evaluation CSV saved → {(Path(args.output) / 'eval_manual.csv').resolve()}")
